# Experiment 2: Delete an Arbitrary Constituent
...
However, we also provide the scripts and results of Experiment 2. You can replicate the results in our paper by using our processed data, or you can extract the annotations of treebanks and process by yourself.
...
```bash
python process_ptb.py --process random --output_path exp2/stimulus/
python process_ctb.py --process random --output_path exp2/stimulus/
```

And run the script in `exp2/` to construct the splitted tests for participants:
```bash
