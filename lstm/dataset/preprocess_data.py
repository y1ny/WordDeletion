...
class PreprocessData:
    ...
    def _handle_glove(self):
        ...
        if os.path.exists(os.path.split(self._glove_path)[0] + '/processed_glove.pkl',):
            logger.info(f"read processed glove from: {os.path.split(self._glove_path)[0] + '/processed_glove.pkl'}")
            self._word2vec = read_pkl(os.path.split(self._glove_path)[0] + '/processed_glove.pkl')
            ...
            save_pkl(os.path.split(self._glove_path)[0] + '/processed_glove.pkl', self._word2vec)
    ...
    def run(self):
        ...
        logger.info('read word deletion json...')
        ...
        self._data['test'] = {
            'context': dict2array(test_cache_nopad['context']),
            'question': dict2array(test_cache_nopad['question']),
