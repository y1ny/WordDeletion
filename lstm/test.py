...
import logging
import argparse
from ReWrite_public.LSTM.dataset.deletion_dataset import DeletionDataset
from models import *
...
def test(config_path, out_path, id, n_sample):
    ...
    model = PointLSTMwithPosition(dataset_h5_path)
    ...
    model_weight_path = global_config['data']['model_path'].split('.')[0] + f'_{n_sample}_{id}.pt'
    ...
    if out_path is None:
        ...
        # done: 明天要把ans_id存一下，目前只存了ans_text
        ...
        samples_id = samples_id.tolist()
        # samples_id = list(map(lambda x: x.decode(), samples_id))
    ...
    for bnum, batch in enumerate(batch_data):
        ...
        for c, a in tmp_context_ans:
            ...
            for j, word in enumerate(tmp_ans):
                ...
                if cur_space[j]:
                    cur_ans += ' '
            assert (a[1]-a[0]) + 1 == len(cur_ans.strip().split(' '))
            answer.append([cur_ans.strip(), str(a[0]), str(a[1])])
            i += 1
        cnt += i
