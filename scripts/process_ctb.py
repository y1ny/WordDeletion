...
def ctb_constituent_analysis(result, cons_feature_dict, demon = None):
    ...
    for idx, r in enumerate(result):
        ...
        word_node = []
        combs = []
        combs.extend(process_ptb.combine(cons_feature[0], 1))
        combs.extend(process_ptb.combine(cons_feature[0], 2))
        combs.extend(process_ptb.combine(cons_feature[0], 3))
        ...
        if demon == None:
            ...
            if word_node == [] and phrase_node != []:
                ...
                if phrase_node != []:
                    ...
                    if word_node == []:
                        char_sent.append([r[0], r[2], [], [], demon[idx]])
                    else:
                        word_sent.append([r[0], r[2], phrase_node, word_node, demon[idx]])
...
import random
def random_delete_XP_zh(cons_feature_dict):
    ...
    sent_pool = []
    # only keep the sentence whose depth is within 4~10
...
if __name__ == '__main__':
    ...
    if args.process == 'filter':
        ...
        for lst in depth_lst:
            ...
            for sent in lst.keys():
                ...
                for NP in NP_lst:
                    ...
                    tree_distance = process_ptb.get_node_distance(NP, par, cons_feature)
        ...
        cons_feature_dict = read_pkl("data/ctb_cons_feature.pkl")
        random_test = random_delete_XP_zh(cons_feature_dict)
