...
class ModelArguments:
    model_name_or_path: Optional[str] = field(default="/path/to/gpt2-chinese-cluecorpussmall")
...
class DataArguments:
    ...
    n_training_sample: int = field(
        default=700, metadata={"help": "Path to the training data."}
    )
    n_iter: int = field(
...
def preprocess(
    ...
    start_char = list(msg[2])[:msg[4]]
    
    answer_start = len("".join(start_char))
    answer_end = answer_start + len(msg[3]) - 1
    start_positions = []
    end_positions = []
    offset_mapping = encoding.pop("offset_mapping")
    for i, offset in enumerate(offset_mapping):
        if encoding['token_type_ids'][i] != 1:
...
def train():
    ...
    if training_args.from_scratch == 'all':
        ...
        print(f"loading model from path: {model_args.model_name_or_path}")
