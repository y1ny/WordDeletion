...
from dataset.deletion_dataset import DeletionDataset
from models import *
...
def train(config_path, id, n_sample):
    ...
    dataset = DeletionDataset(global_config)
    # print(dataset[80])

    logger.info('constructing model...')
    dataset_h5_path = global_config['data']['dataset_h5']

    model = PointLSTMwithPosition(dataset_h5_path, only_position=global_config['global']['only_positon'])
    ...
    global_config['data']['model_path'] = global_config['data']['model_path'].split('.')[0] + f'_{n_sample}_{id}.pt'
    global_config['data']['checkpoint_path'] = global_config['data']['checkpoint_path'] + f'_{n_sample}_{id}'
    ...
    # to maintain the consistent parameter-updating steps.
    up_epoch = 20
    n_update_step = up_epoch*(800/32)
    if n_sample < train_batch_size:
        n_epoch = int(n_update_step)
    else:
        n_epoch = int(n_update_step / (n_sample / train_batch_size))
    global_config['train']['epoch'] = n_epoch
