...
def random_xp_retriever(target, cons_feature_dict, is_fronting = False):
    ...
    for sent in cons_feature_dict.keys():
        ...
        for node in cons_feature_dict[sent][-2].keys():
            ...
            if len(cons_feature_dict[sent][2][node]) == len(sent.split(' ')):
                continue
            if is_fronting:
                if cons_feature_dict[sent][2][node][0][0] == 0:
                    continue
...
def measure_left_right_size(parsed_lst):
    ...
    for idx, parsed in enumerate(parsed_lst):
        ...
        while True:
            ...
            if tmp == []:
                break
        # display_tree(parsed, target_lst[idx])
        # print(left, right)
