...
class PointLSTMwithPosition(torch.nn.Module):
    """
    pointer lstm model for deletion task
    Args:
        - global_config: model_config with types dictionary

    Inputs:
        context: (batch, seq_len)
        # question: (batch, seq_len), deletion task do not need question
        # the task formulation is : given context (John likes the cat), predict the deleted sentence (John likes)
        
        # char-level is not used in origin Match LSTM
    ...
    def __init__(self, dataset_h5_path, only_position = False):
        super(PointLSTMwithPosition, self).__init__()
        ...
        # construct model
        self.embedding = GloveEmbeddingwithPosition(dataset_h5_path=dataset_h5_path, only_position = only_position)
    ...
    def forward(self, context, question, context_char=None, question_char=None, context_f=None, question_f=None):
        ...
        ans_range_prop = self.pointer_net.forward(context_encode, context_mask)
        ...
        return ans_range_prop, ans_range, None
