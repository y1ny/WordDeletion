...
class GloveEmbeddingwithPosition(torch.nn.Module):
    """
    ...
    def __init__(self, dataset_h5_path, only_position = False):
        super(GloveEmbeddingwithPosition, self).__init__()
        ...
        self.embedding_layer = torch.nn.Embedding(num_embeddings=n_embeddings, embedding_dim=len_embedding,
                                                  _weight=weights)
        self.position_enc = torch.nn.Embedding(max_len + 1, len_embedding, padding_idx=0)
        self.position_enc.weight.data = position_encoding_init(max_len + 1, len_embedding)
    ...
    def get_absolute_pos(self, word_sequences):
        ...
        batch = torch.cat(batch).view(word_sequences.shape[0],word_sequences.shape[1])        
    ...
    def forward(self, x):
        ...
        word_pos = self.get_absolute_pos(x)
        word_pos = word_pos.to(x.device)
        word_pos_encoded = self.position_enc(word_pos)
        
        # 仅保留position embedding作为输入
        if not self.only_position:
            word_pos_encoded = tmp_emb + word_pos_encoded
        
        out_emb = word_pos_encoded.transpose(0, 1)
...
class PositionEmbedding(torch.nn.Module):
    ...
    def __init__(self, dataset_h5_path):
        ...
        self.dataset_h5_path = dataset_h5_path
        n_embeddings, len_embedding, weights = self.init()
    ...
    def forward(self, Hp, Hp_mask, Hq, Hq_mask):
        ...
        if self.bidirectional:
            ...
            if self.gated_attention:
                ...
                rtn_para['right'] = {'alpha': right_alpha}
